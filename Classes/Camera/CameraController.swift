...
public protocol CameraControllerDelegate: AnyObject {
     ...
     */
    func didCreateMedia(_ cameraController: CameraController, media: CameraController.MediaOutput, exportAction: KanvasExportAction, stickers: [MovableView])
...
open class CameraController: UIViewController, MediaClipsEditorDelegate, CameraPreviewControllerDelegate, EditorControllerDelegate, CameraZoomHandlerDelegate, OptionsControllerDelegate, ModeSelectorAndShootControllerDelegate, CameraViewDelegate, CameraInputControllerDelegate, FilterSettingsControllerDelegate, CameraPermissionsViewControllerDelegate, KanvasMediaPickerViewControllerDelegate, MediaPickerThumbnailFetcherDelegate, MultiEditorComposerDelegate {
    ...
    func didFinishExportingVideo(url: URL?) {
        didFinishExportingVideo(url: url, info: MediaInfo(source: .kanvas_camera), archive: nil, action: .previewConfirm, mediaChanged: true, stickers: [])
    }

    func didFinishExportingImage(image: UIImage?) {
        didFinishExportingImage(image: image, info: MediaInfo(source: .kanvas_camera), archive: nil, action: .previewConfirm, mediaChanged: true, stickers: [])
    }
    ...
    public func didFinishExportingVideo(url: URL?, info: MediaInfo?, archive: Data?, action: KanvasExportAction, mediaChanged: Bool, stickers: [MovableView]) {
        ...
        if let asset = asset, let info = info {
            ...
            performUIUpdate { [weak self] in
                if let self = self {
                    ...
                    self.delegate?.didCreateMedia(self, media: [.success(media)], exportAction: action, stickers: stickers)
                }
            }
        } else {
            performUIUpdate { [weak self] in
                if let self = self {
                    self.existingEditor?.hideLoading()
                    self.handleCloseSoon(action: action)
                    self.delegate?.didCreateMedia(self, media: [.failure(CameraControllerError.exportFailure)], exportAction: action, stickers: [])
                }
            }
        }
    }

    public func didFinishExportingImage(image: UIImage?, info: MediaInfo?, archive: Data?, action: KanvasExportAction, mediaChanged: Bool, stickers: [MovableView]) {
        guard settings.features.multipleExports == false else { return }
    ...
    public func didFinishExportingFrames(url: URL?, size: CGSize?, info: MediaInfo?, archive: Data?, action: KanvasExportAction, mediaChanged: Bool) {
        ...
        performUIUpdate {
            ...
            self.delegate?.didCreateMedia(self, media: [.success(media)], exportAction: action, stickers: [])
        }
    }
    ...
    private func exportingDidFinishHandler(from result: [Result<EditorViewController.ExportResult, Error>]) async {
        Task { [weak self] in
            ...
            self.exportCancellable = publishers.receive(on: DispatchQueue.main).sink { completion in
                ...
                self.delegate?.didCreateMedia(self, media: items, exportAction: .post, stickers: [])
            }
        }
    }
